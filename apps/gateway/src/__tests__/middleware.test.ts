...
function makeConfig(cors: { enabled: boolean; origins: string[] }): GatewayConfig {
  return {
    ...
    database: {
      ...
      database: 'test', user: 'test', password: 'test', ssl: false, dataDir: '',
    },
    admin: { password: 'test' },
...
describe('errorHandler middleware', () => {
  ...
  test('catches AppError thrown in handler via onError', async () => {
    ...
    app.get('/fail', () => { throw new AppError(400, 'Invalid input', 'BAD_INPUT'); });
  ...
  test('catches generic Error via onError and returns 500', async () => {
    ...
    app.get('/fail', () => { throw new Error('Unexpected crash'); });

  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 text-muted-foreground">
        {attempt.ttfbMs != null && <span>TTFB {formatMs(attempt.ttfbMs)}</span>}
...
export function ConversationTraceSheet({
  conversationId,
  open,
  onOpenChange,
}: ConversationTraceSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full max-w-full md:w-[560px] md:max-w-[560px] p-0 flex flex-col"
        hideCloseButton
      >
        <SheetTitle className="sr-only">对话链路追踪</SheetTitle>
        <SheetDescription className="sr-only">
          查看完整对话中每轮请求和 Failover 链路
        </SheetDescription>

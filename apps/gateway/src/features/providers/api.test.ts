...
describe('providers API', () => {
  ...
  it('PUT /api/providers/:id returns 200', async () => {
    ...
    const res = await authPut(ctx, `/api/providers/${createBody.data.id}`, { name: 'Updated Provider' });
  ...
  it('PATCH /api/providers/:id/toggle flips enabled state', async () => {
    ...
    const toggleRes = await authenticatedRequest(ctx.app, 'PATCH', `/api/providers/${createBody.data.id}/toggle`, ctx.token);
    const { status: toggleStatus, body: toggleBody } = await parseJson<{ enabled: boolean }>(toggleRes);

    expect(toggleStatus).toBe(200);
    expect(toggleBody.success).toBe(true);
    expect(toggleBody.data.enabled).toBe(false);

    const toggleRes2 = await authenticatedRequest(ctx.app, 'PATCH', `/api/providers/${createBody.data.id}/toggle`, ctx.token);
    const { status: toggleStatus2, body: toggleBody2 } = await parseJson<{ enabled: boolean }>(toggleRes2);

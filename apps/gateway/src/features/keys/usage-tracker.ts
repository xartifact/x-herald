...
import { virtualKeys, keyUsageDaily } from './db';

/**
 * 更新密钥使用统计（异步，不阻塞请求）
 * 在请求完成后调用
 */
export async function trackKeyUsage(params: {
  keyId: string;
  inputTokens: number;
  outputTokens: number;
  ...
  try {
    ...
    const doTrack = async (trx: DbClient) => {
      ...
      await trx.update(virtualKeys).set({
        lastUsedAt: new Date(),
      ...
      await trx.insert(keyUsageDaily).values({
        keyId: params.keyId,
        date: today,
        requestCount: 1,

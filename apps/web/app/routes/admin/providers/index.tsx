...
function providerToForm(p: Provider): ProviderFormData {
  return {
    name: p.name, apiKey: '', enabled: p.enabled,
    protocols: {
      openai: { enabled: !!p.protocols.openai?.enabled, baseUrl: p.protocols.openai?.baseUrl ?? '' },
      anthropic: { enabled: !!p.protocols.anthropic?.enabled, baseUrl: p.protocols.anthropic?.baseUrl ?? '' },
      gemini: { enabled: !!p.protocols.gemini?.enabled, baseUrl: p.protocols.gemini?.baseUrl ?? '' },
    },
  }
}
...
export function ProvidersPage() {
  ...
  const handleSubmit = useCallback((data: ProviderFormData) => {
    ...
    const payload = { name: data.name, apiKey: data.apiKey || undefined, protocols, enabled: data.enabled }

...
import { ModelNotFoundError, ModelDisabledError, NoAvailableInstanceError, NoSuitableInstanceError, RequestRejectedError } from './router-selector';
...
describe('filterCandidates', () => {
  ...
  it('should exclude instance when circuit breaker is open', async () => {
    ...
    const result = await filterCandidates(
      [{ instance: openInstance, provider }, { instance: okInstance, provider }],
  ...
  it('should exclude instance with status "down"', async () => {
    ...
    const result = await filterCandidates(
      [{ instance: downInstance, provider }, { instance: okInstance, provider }],
      defaultRoutingContext,
      group,
    );
  ...
  it('should exclude instance when provider protocol is disabled', async () => {
    const instance = createTestInstance();
    const provider = createTestProvider({ protocols: { openai: { baseUrl: 'https://api.openai.com', enabled: false } } });
    const group = createTestGroup();
  ...
  it('should return empty array when all instances are filtered out', async () => {
    ...
    const result = await filterCandidates(
      [{ instance: instance1, provider }, { instance: instance2, provider }],
      defaultRoutingContext,
      group,
    );
...
describe('selectByStrategy', () => {
  ...
  function createCandidate(overrides: { instance?: Partial<ModelInstance>; provider?: Partial<ProviderSelect> } = {}) {
    return {
  ...
  describe('priority strategy (default)', () => {
    ...
    it('should sort by createdAt when priority is equal', async () => {
      const older = createCandidate({ instance: { priority: 0, createdAt: new Date('2025-01-01') } });
      const newer = createCandidate({ instance: { priority: 0, createdAt: new Date('2025-06-01') } });
  ...
  describe('cost_optimized strategy', () => {
    it('should sort candidates by total cost ascending', async () => {
      const expensive = createCandidate({ instance: { costPer1kTokens: { input: 0.03, output: 0.06 }, name: 'Expensive', priority: 0 } });
      const cheap = createCandidate({ instance: { costPer1kTokens: { input: 0.01, output: 0.02 }, name: 'Cheap', priority: 0 } });
    ...
    it('should place candidates without cost data after those with cost', async () => {
      const withCost = createCandidate({ instance: { costPer1kTokens: { input: 0.01, output: 0.01 }, name: 'WithCost', priority: 0 } });
      const noCost = createCandidate({ instance: { costPer1kTokens: null, name: 'NoCost', priority: 0 } });
  ...
  describe('least_response_time strategy', () => {
    it('should sort candidates by ttfb ascending when perf data exists', async () => {
      ...
      (fetchGroupInstancesPerf as ReturnType<typeof mock>).mockImplementation(async () => {
        ...
        map.set('slow', { ttfbAvg: 2000, ttfbP95: null, latencyAvg: null, successRate: null, avgRetryCount: null });
        map.set('fast', { ttfbAvg: 200, ttfbP95: null, latencyAvg: null, successRate: null, avgRetryCount: null });
        return map;
      });
    ...
    it('should place candidates without perf data after those with perf', async () => {
      ...
      (fetchGroupInstancesPerf as ReturnType<typeof mock>).mockImplementation(async () => {
        const map = new Map<string, InstancePerfData>();
        map.set('with-perf', { ttfbAvg: 500, ttfbP95: null, latencyAvg: null, successRate: null, avgRetryCount: null });
        return map;
      });
  ...
  describe('smart strategy', () => {
    it('should sort candidates by smart score descending', async () => {
      ...
      (fetchGroupInstancesPerf as ReturnType<typeof mock>).mockImplementation(async () => {
        const map = new Map<string, InstancePerfData>();
        map.set('good', { ttfbAvg: 200, ttfbP95: null, latencyAvg: null, successRate: 0.99, avgRetryCount: 0.1 });
        map.set('bad', { ttfbAvg: 3000, ttfbP95: null, latencyAvg: null, successRate: 0.5, avgRetryCount: 4 });
        return map;
      });

...
export function AiAssistPage() {
  ...
  const diagnoseMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const apiError = error as { data?: { error?: string; code?: string } }
      toast.error(apiError.data?.error || '诊断请求失败')
    },
  })
                ...
                <SelectContent>
                  {failureLogs.map((log) => (
                    <SelectItem key={log.id} value={log.id}>
                      <span className="truncate max-w-[320px]" title={log.id}>
                        {log.modelName} · {log.errorType || 'unknown'} · {new Date(log.createdAt).toLocaleString()}
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>

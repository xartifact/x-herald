...
async function loadClientModelsPage() {
  const mod = await import('../index')
  return mod.ClientModelsPage as React.FC
}
...
describe('ClientModelsPage', () => {
  ...
  test('exports ClientModelsPage as a function', async () => {
    const ClientModelsPage = await loadClientModelsPage()
    expect(typeof ClientModelsPage).toBe('function')
  })
  ...
  test('shows loading state when query is loading', async () => {
    ...
    const element = React.createElement(ClientModelsPage)
    expect(element).toBeDefined()
    expect(element.type).toBe(ClientModelsPage)
  })
  ...
  test('shows empty state when no providers have enabled protocols', async () => {
    const ClientModelsPage = await loadClientModelsPage()
    _queryStates = {
      providers: {
        isLoading: false,
  ...
  test('shows models when API returns providers with enabled protocols', async () => {
    const ClientModelsPage = await loadClientModelsPage()
    _queryStates = {
      providers: {
        isLoading: false,
  ...
  test('filters disabled provider protocols', async () => {
    const ClientModelsPage = await loadClientModelsPage()
    _queryStates = {
      providers: {
        isLoading: false,
        data: [
          {
            ...
            protocols: {
              openai: { baseUrl: 'https://api.openai.com', enabled: true },
              anthropic: { baseUrl: 'https://api.anthropic.com', enabled: false },
              gemini: { baseUrl: 'https://gemini.com', enabled: true },
            },
            enabled: true,
            createdAt: '2024-01-01T00:00:00Z',
            updatedAt: '2024-01-01T00:00:00Z',

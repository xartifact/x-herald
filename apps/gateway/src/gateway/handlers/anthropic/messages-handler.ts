...
export async function handleAnthropicMessages(
  ...
  const incomingProtocol = 'anthropic' as const
  let retryCount = 0

  try {
    const rawBody = (preprocessedBody ?? (await c.req.json())) as {
      model?: string
      [key: string]: unknown
    }
    ...
    try {
      for (let i = 0; i < candidates.length; i++) {
        ...
        if (i > 0)
          logger.info(
            { requestId, instanceId: instance.id, instanceName: instance.name, candidateIdx: i },
        ...
        const executor = new AnthropicMessagesExecutor({
          c,
          ctx,
          candidate: { instance, provider, group, matchedRule, mapping, decision },
          req,
          abortManager,
          providerUrl,

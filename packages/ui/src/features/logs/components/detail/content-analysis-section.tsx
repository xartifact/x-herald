...
export function ContentAnalysisSection({ log, contentFeatures }: ContentAnalysisSectionProps) {
  if (!(log.metadata?.content || contentFeatures?.request || contentFeatures?.response || contentFeatures?.complexity)) {
    return null
  }

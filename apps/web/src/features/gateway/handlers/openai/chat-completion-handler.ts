...
import { ChatCompletionCandidateExecutor } from './chat-completion-executor';
...
export async function handleOpenAIChatCompletion(
  ...
  try {
    const rawBody = preprocessedBody as { model?: string; [key: string]: unknown } ??
      (await c.req.json()) as { model?: string; [key: string]: unknown };

    logger.info({ requestId, model: rawBody.model, protocol: incomingProtocol }, 'Processing chat completion');
    ...
    try {
      for (let i = 0; i < candidates.length; i++) {
        ...
        if (!providerUrl) {
          ...
          return c.json({ error: { type: 'protocol_error', message: `Protocol '${targetProtocol}' not configured for provider` } }, 400);
        }

...
export function useCreateAccessModel() {
  ...
  return useMutation({
    mutationFn: async (data: CreateAccessModelPayload) => {
      const response = await post<ApiResponse<AccessModel>>('/api/access-models', data as unknown as Record<string, unknown>, {
        extractData: false,
      })
      if (!response.success) {
...
export function useUpdateAccessModel() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: UpdateAccessModelPayload }) => {
      const response = await put<ApiResponse<AccessModel>>(`/api/access-models/${id}`, data as unknown as Record<string, unknown>, {
        extractData: false,
      })
      if (!response.success) {
...
export function useDeleteAccessModel() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await deleteRequest<ApiResponse<AccessModel>>(

...
describe('getLayoutedElements', () => {
  ...
  test('sourceHandle grouping in TB mode: branches with different sourceHandles are laid out', () => {
    ...
    const edges = [createEdge('A', 'B', 'true'), createEdge('A', 'C', 'false')]
    const result = getLayoutedElements(nodes, edges, 'TB')
  ...
  test('sourceHandle grouping in LR mode: branches with different sourceHandles are laid out', () => {
    ...
    expect(layoutedB.sourcePosition).toBe(Position.Right)
    expect(layoutedB.targetPosition).toBe(Position.Left)
    expect(layoutedC.sourcePosition).toBe(Position.Right)
    expect(layoutedC.targetPosition).toBe(Position.Left)
  })
})

...
export class SlidingWindowCounter {
  ...
  record(): { allowed: boolean; remaining: number; resetAt: number } {
    ...
    if (!decision.allowed) {
      return { allowed: false, remaining: 0, resetAt: decision.resetAt }
    }
...
export class DailyAccumulator {
  ...
  private getNextMidnight(): number {
    return getNextMidnightMs(this.now())
  }

  private checkReset(): void {
    if (shouldResetDaily({ now: this.now(), resetAt: this.resetAt })) {
...
export class RateLimitEngine {
  ...
  private getOrCreateCounters(keyId: string, config: RateLimitConfig): KeyCounters {
    ...
    if (config.rpd && !counters.rpd) {
      counters.rpd = new SlidingWindowCounter(86_400_000, config.rpd, this.now)

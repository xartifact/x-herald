...
function createParams(overrides: Partial<ResponseHandlerParams> = {}): ResponseHandlerParams {
  return {
    c: createMockContext(),
    response: new Response('data: {"id":"test","choices":[{"delta":{"content":"hi"}}]}\n\ndata: [DONE]\n\n', {
...
describe('handleStreamingResponse', () => {
  ...
  it('returns stream with same-protocol passthrough (no transformation)', async () => {
    ...
    const params = createParams({
      incomingProtocol: 'openai',
      targetProtocol: 'openai',
      response: new Response(createSSEStream(events), { headers: { 'content-type': 'text/event-stream' } }),
    });
  ...
  it('applies model remap stream when isMapped=true', async () => {
    const events = [
      { id: '1', model: 'backend-model', choices: [{ delta: { content: 'hello' } }] },
    ];
  ...
  it('finalizes log as failure and marks stream aborted on client disconnect', async () => {
    ...
    const params = createParams({
      ...
      request: new Request('http://localhost/v1/chat/completions', { signal: abortController.signal }),
    });

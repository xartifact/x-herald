...
import { LogSheetToolbar } from './LogSheetToolbar'
...
export function LogDetailSheet({ log, open, onOpenChange, formatDuration, formatTokens }: LogDetailSheetProps) {
  if (!log) return null
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full max-w-full md:w-[85vw] md:max-w-[85vw] 2xl:w-[70vw] 2xl:max-w-[70vw] p-0 flex flex-col gap-0" hideCloseButton>
        ...
        <LogSheetToolbar log={log} isPending={isPending} isSuccess={isSuccess} onClose={() => onOpenChange(false)} />
        ...
        <LogSheetStatusBar log={log} isPending={isPending} isSuccess={isSuccess} formatDuration={formatDuration} formatTokens={formatTokens} />
      </SheetContent>
    </Sheet>
  )
}

...
providersRoutes.post('/', async (c) => {
  try {
    ...
    if (!parsed.success) return c.json({ error: parsed.error.issues[0].message, code: 'VALIDATION_ERROR' }, 400);
...
providersRoutes.get('/:id/thinking-type-mappings', async (c) => {
  try {
    ...
    return c.json({ success: true, data: result.mappings, syntheticThinking: result.syntheticThinking });
  } catch (error) {
    logger.warn({ err: error }, 'Failed to get thinking type mappings');
    return c.json({ error: 'Failed to get thinking type mappings', code: 'MAPPINGS_GET_ERROR' }, 500);
  }
});
...
providersRoutes.put('/:id/thinking-type-mappings', async (c) => {
  try {
    ...
    if (!result) return c.json({ error: 'Provider not found', code: 'PROVIDER_NOT_FOUND' }, 404);
    return c.json({ success: true, message: 'Thinking type mappings updated successfully', data: result });
  } catch (error) {
    logger.warn({ err: error }, 'Failed to update thinking type mappings');
    return c.json({ error: 'Failed to update thinking type mappings', code: 'MAPPINGS_UPDATE_ERROR' }, 500);
  }
});
...
providersRoutes.get('/:id/models', async (c) => {
  try {
    ...
    if (!result.ok) {
      if (result.code === 'NOT_FOUND') return c.json({ error: 'Provider not found', code: 'PROVIDER_NOT_FOUND' }, 404);
      return c.json({ error: 'Provider is disabled', code: 'PROVIDER_DISABLED' }, 400);
    }
    return c.json({ success: true, data: result.models, total: result.models.length, fetchError: result.fetchError });
  } catch (error) {
...
providersRoutes.post('/:id/sync-models', async (c) => {
  try {
    ...
    if (!result.ok) {
      const codes = { NOT_FOUND: 'PROVIDER_NOT_FOUND', DISABLED: 'PROVIDER_DISABLED', GROUP_NOT_FOUND: 'MODEL_GROUP_NOT_FOUND' } as const;
      return c.json({ error: result.code, code: codes[result.code] }, result.code === 'NOT_FOUND' ? 404 : 400);
    }

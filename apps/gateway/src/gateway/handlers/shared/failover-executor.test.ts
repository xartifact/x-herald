...
function createParams(overrides: Partial<FailoverExecutorParams> = {}): FailoverExecutorParams {
  ...
  const handleGatewayError = mock(async () => new Response('gateway error', { status: 504 }))
  const handleProviderError = mock(async () => new Response('provider error', { status: 500 }))
...
function getMarkLogCalls(params: FailoverExecutorParams): MarkLogFailedParams[] {
  return getMockCalls(params.onMarkLogAsFailed).map((call) => call[0] as MarkLogFailedParams)
}
...
describe('executeFailoverIteration', () => {
  ...
  describe('HTTP failover path', () => {
    ...
    it('does not failover for 400 status (not in FAILOVER_STATUS_CODES)', async () => {
      ...
      expect(params.onRecordFailure).not.toHaveBeenCalled()
      expect(params.handleProviderError).toHaveBeenCalledWith(response, params.rawBody)
    })
  ...
  describe('provider error path', () => {
    ...
    it('returns error via handleProviderErrorPassthrough when passthrough enabled', async () => {
      ...
      const result = await executeFailoverIteration(params)
      expect(result.type).toBe('error')
      expect(params.handleProviderErrorPassthrough).toHaveBeenCalledWith(response, params.rawBody)

...
describe('access-models API', () => {
  ...
  it('POST /api/access-models with valid data returns 201', async () => {
    ...
    const res = await authPost(ctx, '/api/access-models', {
      ...
      enabled: true,
      capabilities: { streaming: true, functionCalling: false, vision: false, jsonMode: false, maxTokens: 4096, contextWindow: 8192 },
    });
    const { status, body } = await parseJson<AccessModel>(res);
  ...
  it('PUT /api/access-models/:id updates displayName and enabled', async () => {
    ...
    const res = await authPut(ctx, `/api/access-models/${id}`, { displayName: 'Updated Name', enabled: false });
    const { status, body } = await parseJson<AccessModel>(res);
    expect(status).toBe(200);
    expect(body.success).toBe(true);
  ...
  it('PUT /api/access-models/:id nonexistent returns 404', async () => {
    const res = await authPut(ctx, '/api/access-models/00000000-0000-0000-0000-000000000000', { displayName: 'nope' });
    const { status, body } = await parseJson<AccessModel>(res);
    expect(status).toBe(404);
    expect(body.success).toBe(false);
  });
  ...
  it('PATCH /api/access-models/:id/toggle nonexistent returns 404', async () => {
    const res = await authPatch(ctx, '/api/access-models/00000000-0000-0000-0000-000000000000/toggle');
    const { status, body } = await parseJson<AccessModel>(res);
    expect(status).toBe(404);
    expect(body.success).toBe(false);
  });

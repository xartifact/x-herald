...
export class ChatCompletionCandidateExecutor {
  ...
  private async buildProviderParts(): Promise<{ transformedBody: unknown; targetUrl: string; pHeaders: Record<string, string> }> {
    ...
    if (isPassthroughEnabled) {
      return { transformedBody: { ...(rawBody || {}), model: instance.actualModelName }, targetUrl: joinUrl(providerUrl, getEndpoint(targetProtocol, isStreaming)), pHeaders: { ...filtered, 'authorization': `Bearer ${provider.apiKey}` } };
    }
    ...
    let pHeaders: Record<string, string> = { ...filtered, ...Object.fromEntries(Object.entries(adapted.headers || {}).map(([k, v]) => [k.toLowerCase(), v])), 'authorization': `Bearer ${provider.apiKey}` };
  ...
  private get candidate(): CandidateInfo { return this.config.candidate; }
  private get req(): RequestContext { return this.config.req; }

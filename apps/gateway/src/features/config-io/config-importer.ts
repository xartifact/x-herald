...
export async function importConfig(data: EngineExportFormat['data']): Promise<EngineImportResult> {
    ...
    try {
      ...
      if (existing.length > 0) {
        await db
          ...
          .set({
            apiKey: p.apiKey,
            protocols: p.protocols as never,
            enabled: p.enabled,
            updatedAt: new Date(),
          })
    ...
    try {
      ...
      if (existing.length > 0) {
        ...
        await db
          ...
          .values({ key: c.key, value: c.value as never, description: c.description })
  ...
  for (const i of data.modelInstances) {
    try {
      ...
      if (!providerId) {
        errors.push(
          `ModelInstance "${i.name}": provider "${i.providerName}" not found after import`,
        )
      ...
      const targetGroupNames =
        i.groupNames && i.groupNames.length > 0 ? i.groupNames : i.groupName ? [i.groupName] : []
      ...
      if (resolvedGroupIds.length > 0) {
        ...
        await db
          ...
          .values(resolvedGroupIds.map((gid) => ({ groupId: gid, instanceId })))
      }
    } catch (err) {

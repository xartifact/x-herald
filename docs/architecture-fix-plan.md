...
- **问题**：keys/model-groups 从 `@xartifact/x-herald-gateway` 导入类型，providers/logs 从 `@xartifact/x-herald-shared` 导入
- **改动**：全部改为从 `@xartifact/x-herald-shared` 导入
- **验证**：`grep -r "from.*@xartifact/x-herald-gateway" apps/web/ packages/ui/src/` 无类型导入
...
- 同包内 `@xartifact/x-herald-gateway` 自引用改为相对路径

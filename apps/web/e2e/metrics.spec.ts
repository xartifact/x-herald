...
test.describe('Metrics', () => {
  ...
  test('shows summary cards section', async ({ page }) => {
    await page.goto('/admin/metrics')
    await page.waitForLoadState('networkidle')
    const summaryExists = await page.getByText(/ttfb|tps|延迟|响应时间|perf/i).first().isVisible().catch(() => false)
    if (summaryExists) {
  ...
  test('instance performance chart renders when data exists', async ({ page }) => {
    await page.goto('/admin/metrics')
    await page.waitForLoadState('networkidle')
    const chartArea = page.locator('[class*="recharts"], [class*="chart"], canvas, svg.g-rect').first()

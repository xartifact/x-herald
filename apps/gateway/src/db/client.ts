...
async function createPgliteDatabase(dataDir: string): Promise<PostgresDb> {
  ...
  if (MIGRATE_ON_BOOT) {
    await runPgliteMigrations(pgliteClient, dataDir);
  } else {
    logger.info('[DB] MIGRATE_ON_BOOT=false，跳过自动迁移');
  }

  return getDbClient()!;
}

async function runPgliteMigrations(pgliteClient: { exec: (sql: string) => Promise<unknown>; query: (sql: string, params?: unknown[]) => Promise<{ rows: Array<{ hash: string }> }> }, _dataDir: string) {
  const { createHash } = await import('crypto');
  const fs = await import('fs');
  const { extname } = await import('path');

...
export class AnthropicMessagesExecutor {
  ...
  async prepareRequest(): Promise<PreparedRequest> {
    ...
    this.ctx.instanceConfig = instance.config ?? undefined;
  ...
  private async buildProviderParts(): Promise<{ transformedBody: unknown; targetUrl: string; pHeaders: Record<string, string> }> {
    ...
    if (this.ctx.instanceConfig?.customHeaders) pHeaders = buildHeaders(pHeaders, this.ctx.instanceConfig.customHeaders, this.ctx);

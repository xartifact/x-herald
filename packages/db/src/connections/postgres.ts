...
async function createDatabaseIfNotExists(options: DatabaseOptions, logger: DbLogger): Promise<void> {
...
async function runPostgresMigrations(
  ...
  const migrationFiles = fs.readdirSync(migrationsFolder).filter((f) => f.endsWith(".sql")).sort();
  logger.info({ fileCount: migrationFiles.length, appliedCount: appliedHashes.size }, "[DB] Scanning migrations");

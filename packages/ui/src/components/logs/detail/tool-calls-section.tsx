...
import { JsonViewer } from '../../admin/JsonViewer'
...
export function ToolCallsSection({ log }: ToolCallsSectionProps) {
  if (!log.metadata?.toolCalls) return null
  ...
  return (
    <Section
      title="工具调用"
      badge={log.toolCallsCount != null && log.toolCallsCount > 0 ? <Badge variant="secondary" className="text-xs">{log.toolCallsCount}</Badge> : undefined}
    >
      {toolCalls.pattern && (
        <InfoRow label="调用模式">

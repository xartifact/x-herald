...
export function ProviderStatsToolbar({
  filter,
  isLoading,
  onFilterChange,
  onRefresh,
}: ProviderStatsToolbarProps) {
  const handleSort = (field: SortField) => {
    if (filter.sortField === field) {
      ...
      onFilterChange({
        ...filter,
        sortField: field,
        sortOrder: field === 'successRate' ? 'desc' : 'asc',
      })
    }
  }
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            ...
            {SORT_BUTTONS.map(({ field, icon: Icon, label }) => (
              ...
              >
                ...
                {filter.sortField === field && (
                  <ArrowUpDown
                    className={cn('ml-1 h-3 w-3', filter.sortOrder === 'desc' && 'rotate-180')}
                  />
                )}
              </Button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <Select
              ...
              onValueChange={(v) => onFilterChange({ ...filter, timeRange: v })}
            >

...
| 包 | 测试文件数 | 通过率 | 备注 |
|---|---|---|---|
| `packages/engine` | 19 | 296/303 pass | 7 fail 来自预先存在的 schema-cleaner 问题 |
| `apps/web` | 15 | — | engine 的重复副本（迁移残留） |
| `apps/tanstack` | 0 | — | 管理前端 SPA 无测试 |
...
| `apps/tanstack` 页面级测试 | 每个管理页面的渲染+交互 |

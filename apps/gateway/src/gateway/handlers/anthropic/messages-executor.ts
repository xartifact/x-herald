...
export class AnthropicMessagesExecutor {
  ...
  private readonly circuitBreakerMeta: { instanceName: string; groupName: string; providerName: string };

  constructor(private readonly config: AnthropicExecutorConfig) {
    const { instance, provider, group } = config.candidate;
    this.circuitBreakerMeta = { instanceName: instance.name, groupName: group.name, providerName: provider.name };
  }
  ...
  async prepareRequest(): Promise<PreparedRequest> {
    ...
    return { url: targetUrl, headers: pHeaders, body: JSON.stringify(transformedBody), isPassthroughEnabled, targetProtocol };
  }

  private async buildProviderParts(): Promise<{ transformedBody: unknown; targetUrl: string; pHeaders: Record<string, string> }> {
    const { instance, provider } = this.candidate;
    ...
    if (isPassthroughEnabled) {
      const passthroughBody: { model?: string; messages?: unknown[]; [key: string]: unknown } = { ...rawBody, model: instance.actualModelName };
      if (Array.isArray(passthroughBody.messages)) {
        passthroughBody.messages = normalizeAnthropicPassthroughMessages(passthroughBody.messages as Array<{ role: string; content: unknown }>);
        const supportsThinking = provider.protocols?.anthropic?.enabled && (instance.config?.supportsThinking === true || instance.actualModelName.includes('claude-3-7-') || instance.actualModelName.includes('claude-4'));
        if (supportsThinking && hasAssistantMessagesWithoutThinking(passthroughBody.messages as Array<{ role: string; content: unknown }>)) {
          passthroughBody.messages = injectSyntheticThinkingBlocks(passthroughBody.messages as Array<{ role: string; content: unknown }>);
        }
      }
      return { transformedBody: passthroughBody, targetUrl: joinUrl(providerUrl, getEndpoint(targetProtocol, isStreaming)), pHeaders: { ...filtered, 'x-api-key': provider.apiKey || '' } };
    }
    ...
    const adaptedHeaders = Object.fromEntries(Object.entries(adapted.headers || {}).map(([k, v]) => [k.toLowerCase(), v]));
    const authHeader: Record<string, string> = targetProtocol === 'anthropic' ? { 'x-api-key': provider.apiKey || '' } : { 'authorization': `Bearer ${provider.apiKey}` };
    let pHeaders = { ...filtered, ...adaptedHeaders, ...authHeader };
    const hdrs = this.ctx.instanceConfig as Record<string, unknown> | undefined; if (hdrs?.customHeaders) pHeaders = buildHeaders(pHeaders, hdrs.customHeaders as Record<string, string>, this.ctx);
    return { transformedBody: adapted.body, targetUrl: adapted.url || joinUrl(providerUrl, getEndpoint(targetProtocol, isStreaming)), pHeaders };
  }
  ...
  beforeFetch(): void {
    ...
    if (isStreaming && this.logId) {
      logEventBus.emitLog({
        event: 'waiting',
        logId: this.logId,
        modelName: mapping.modelName || mapping.originalModel || (rawBody?.model as string) || 'unknown',
  ...
  async retry(attempt: number, delay: number, lastResponse?: Response): Promise<void> {
    logger.info({ requestId: this.config.req.requestId, attempt, statusCode: lastResponse?.status, retryDelay: delay }, '[Retry] Retrying upstream request');
    await circuitBreakerRegistry.recordFailure(this.config.candidate.instance.id, this.circuitBreakerMeta);
  }

  async recordFailure(): Promise<void> { await circuitBreakerRegistry.recordFailure(this.config.candidate.instance.id, this.circuitBreakerMeta); }
  async recordSuccess(): Promise<void> { await circuitBreakerRegistry.recordSuccess(this.config.candidate.instance.id, this.circuitBreakerMeta); }
  ...
  private get c(): Context { return this.config.c; }
  private get ctx(): TransformerContext { return this.config.ctx; }
  private get req() { return this.config.req; }
  private get candidate() { return this.config.candidate; }
}

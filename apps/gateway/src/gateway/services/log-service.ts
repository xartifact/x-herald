...
function buildLogInsertValues(params: LogRequestParams, { metadata, inputTokens, outputTokens, toolCallsCount }: LogData): AnyRecord {
  ...
  return {
    ...
    responseBody: params.responseBody,
    errorMessage: params.errorMessage, errorType,
  ...
  try {
    ...
    if (params.logId) {
      ...
      await db.transaction(async (trx) => {
        await trx.update(requestLogs).set({
          status: params.status, statusCode: params.statusCode, responseTimeMs: params.responseTimeMs,
          inputTokens, outputTokens, totalTokens: inputTokens + outputTokens,
          streaming: params.streaming ? 'true' : 'false',
        ...
        if (attemptId && !attemptId.startsWith('temp-')) {
          await trx.update(requestAttempts).set({
    ...
    const logResult = await db.transaction(async (trx) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const insertResult = await (trx.insert(requestLogs) as any).values(buildLogInsertValues(params, logData)).returning({ id: requestLogs.id });

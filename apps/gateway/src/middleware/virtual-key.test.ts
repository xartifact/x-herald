...
let currentRateLimitResult: { allowed: boolean; reason?: string; rpm?: unknown; rpd?: unknown; token?: unknown } | null = null;
...
function setCacheWithKey(key: VirtualKey) {
  const cache = (globalThis as Record<string, unknown>)._vkCache as Map<string, { value: VirtualKey; expiresAt: number }>;

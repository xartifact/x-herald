...
export function MessageAnalysisPanel({ logId, selectedIndices }: MessageAnalysisPanelProps) {
  const { status, result, error, expanded, setExpanded, handleAnalyze, hasSelection, isActive } = useMessageAnalysis({ logId, selectedIndices })
    ...
    <div className="border-b">
      <div className="px-4 py-2.5 bg-muted/30 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          {hasSelection && <Badge variant="secondary" className="text-xs">已选 {selectedIndices.length} 条</Badge>}
        </div>
        <div className="flex items-center gap-2">
          {status === 'done' && (
            <Button variant="ghost" size="sm" onClick={() => handleAnalyze('full')} className="h-7 px-2 text-xs gap-1">
              <RotateCcw className="h-3 w-3" />重新分析
            </Button>
          )}
          <Button
            size="sm"
            onClick={status === 'idle' || status === 'error' ? () => handleAnalyze(hasSelection ? 'selected' : 'full') : undefined}
        ...
        <div className="px-4 py-3 bg-purple-50/30 dark:bg-purple-950/10">
          {status === 'error' ? (
            ...
            <div className="text-sm whitespace-pre-wrap leading-relaxed">
              {result ? JSON.stringify(result, null, 2) : (status === 'loading' ? '正在连接模型...' : '')}
              {status === 'streaming' && <span className="inline-block w-1.5 h-4 bg-foreground/70 animate-pulse ml-0.5 align-middle rounded-sm" />}
            </div>
          )}
        </div>
      )}

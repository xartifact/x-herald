...
function buildLatencySegments(log: Log): Segment[] {
  ...
  if (perf.gatewayOverheadMs) {
    segs.push({ label: '网关预处理', ms: perf.gatewayOverheadMs, color: 'text-blue-600', bgColor: 'bg-blue-500' })
  }
  if (perf.providerTtfbMs) {
    segs.push({ label: 'Provider TTFB', ms: perf.providerTtfbMs, color: 'text-amber-600', bgColor: 'bg-amber-500' })
  }
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1">
        {segments.map((seg) => (
          <div key={seg.label} className="flex items-center gap-1.5 text-[11px]">
            ...
            <span className={cn('font-mono font-medium', seg.color)}>{seg.ms}ms</span>

...
describe('Gemini Protocol Transformer', () => {
  ...
  describe('GeminiTransformer class', () => {
    ...
    it('should delegate normalizeRequest to normalizeGeminiRequest', async () => {
      ...
      await expect(transformer.normalizeRequest({}, ctx)).rejects.toThrow('Gemini normalizeRequest not yet implemented');
    });
    ...
    it('should delegate adaptRequest to adaptGeminiRequest', async () => {
      ...
      await expect(transformer.adaptRequest(request, ctx)).rejects.toThrow('Gemini adaptRequest not yet implemented');
    });

    it('should delegate normalizeResponse to normalizeGeminiResponse', async () => {
      const ctx = createMockContext();
      const response = new Response('{}');
      await expect(transformer.normalizeResponse(response, ctx)).rejects.toThrow('Gemini normalizeResponse not yet implemented');
    });
    ...
    it('should delegate adaptResponse to adaptGeminiResponse', async () => {
      ...
      await expect(transformer.adaptResponse(response, ctx)).rejects.toThrow('Gemini adaptResponse not yet implemented');
    });
    ...
    it('should call adaptStreamToGemini when stream direction is adapt', async () => {
      ...
      await expect(transformer.transformStream(stream, ctx)).rejects.toThrow('Gemini adaptStream not yet implemented');
    });

    it('should call normalizeGeminiStream when no stream direction is set', async () => {
      const ctx = createMockContext();
      const stream = new ReadableStream();
      await expect(transformer.transformStream(stream, ctx)).rejects.toThrow('Gemini normalizeStream not yet implemented');
    });
  });
  ...
  describe('Stub verification', () => {
    ...
    it('normalizeGeminiRequest should throw not yet implemented', () => {
      expect(() => normalizeGeminiRequest({}, ctx)).toThrow('Gemini normalizeRequest not yet implemented');
    });

    it('adaptGeminiRequest should throw not yet implemented', async () => {
      await expect(adaptGeminiRequest({ model: 'test', messages: [] }, ctx)).rejects.toThrow('Gemini adaptRequest not yet implemented');
    });

    it('normalizeGeminiResponse should throw not yet implemented', async () => {
      await expect(normalizeGeminiResponse(new Response('{}'), ctx)).rejects.toThrow('Gemini normalizeResponse not yet implemented');
    });

    it('adaptGeminiResponse should throw not yet implemented', async () => {
      await expect(adaptGeminiResponse({ id: 'test', object: 'chat.completion', created: 0, model: 'test', choices: [] }, ctx)).rejects.toThrow('Gemini adaptResponse not yet implemented');
    });

    it('normalizeGeminiStream should throw not yet implemented', () => {
      expect(() => normalizeGeminiStream(new ReadableStream(), ctx)).toThrow('Gemini normalizeStream not yet implemented');
    });

    it('adaptStreamToGemini should throw not yet implemented', () => {
      expect(() => adaptStreamToGemini(new ReadableStream(), ctx)).toThrow('Gemini adaptStream not yet implemented');
    });
    ...
    it('convertMessage should throw not yet implemented', () => {
      expect(() => convertMessage({ role: 'user', parts: [] })).toThrow('Gemini message converter not yet implemented');
    });

    it('convertToGeminiMessages should throw not yet implemented', () => {
      expect(() => convertToGeminiMessages([])).toThrow('Gemini message converter not yet implemented');
    });

    it('convertGeminiTool should throw not yet implemented', () => {
      expect(() => convertGeminiTool({ functionDeclarations: [] })).toThrow('Gemini tool converter not yet implemented');
    });

    it('convertToGeminiTool should throw not yet implemented', () => {
      expect(() => convertToGeminiTool({ type: 'function', function: { name: 'test' } })).toThrow('Gemini tool converter not yet implemented');
    });
  });
  ...
  describe('Stream routing (transformGeminiStream)', () => {
    it('should call adaptStreamToGemini when direction is adapt', () => {
      ...
      ctx.state.set('streamDirection', 'adapt');
      const stream = new ReadableStream();
      expect(() => transformGeminiStream(stream, ctx)).toThrow('Gemini adaptStream not yet implemented');
    });

    it('should call normalizeGeminiStream when no direction is set', () => {
      const ctx = createMockContext();
      const stream = new ReadableStream();
      expect(() => transformGeminiStream(stream, ctx)).toThrow('Gemini normalizeStream not yet implemented');
    });

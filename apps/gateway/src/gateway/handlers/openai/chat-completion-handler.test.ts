...
mock.module('./chat-completion-executor', () => ({
  ChatCompletionCandidateExecutor: class MockChatCompletionCandidateExecutor {
    ...
    async providerErrorPassthrough() {
      return new Response('passthrough', { status: 500 })
    }
  },
}))
...
function createMockContext(overrides: Record<string, unknown> = {}) {
  const body = (overrides.body as Record<string, unknown>) ?? {
    model: 'gpt-4',
    messages: [{ role: 'user', content: 'hello' }],
  }
...
describe('handleOpenAIChatCompletion', () => {
  beforeEach(() => {
    ...
    const mockRouteCandidates = accessModelRouter.routeCandidates as unknown as {
    ...
    const mockGetProviderUrl = getProviderUrl as unknown as {
      mockImplementation: (fn: (...args: unknown[]) => unknown) => void
    }
    mockGetProviderUrl.mockImplementation(() => 'https://api.openai.com')

    const mockGetTransformer = getTransformer as unknown as {
      mockImplementation: (fn: (protocol: string) => unknown) => void
    }
  ...
  it('dispatches to handleNonStreamingResponse on successful non-streaming candidate', async () => {
    const mockExecuteFailover2 = executeFailoverIteration as unknown as {
      mockImplementation: (fn: (...args: unknown[]) => Promise<unknown>) => void
    }

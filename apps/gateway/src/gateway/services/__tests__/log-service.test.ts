...
const realClientModelRecorder = await import('../../../features/logs/services/client-model-recorder');
...
afterAll(() => {
  ...
  mock.module('../../../features/logs/services/client-model-recorder', () => realClientModelRecorder);
...
describe('logRequest', () => {
  ...
  describe('INSERT path (no logId)', () => {
    ...
    it('should record cost when providerName and tokens > 0', async () => {
      ...
      await logRequest(createBaseParams({ providerName: 'openai', inputTokens: 10, outputTokens: 5 }));
    ...
    it('should NOT record cost when providerName is missing even with tokens', async () => {
      ...
      await logRequest(createBaseParams({ providerName: undefined, inputTokens: 10, outputTokens: 5 }));
    ...
    it('should record cost with estimated tokens when input+output=0', async () => {
      ...
      await logRequest(createBaseParams({ providerName: 'openai', inputTokens: 0, outputTokens: 0 }));
    ...
    it('should call recordClientRequestedModel via dynamic import', async () => {
      const { recordClientRequestedModel } = await import(
  ...
  describe('UPDATE path (with logId)', () => {
    ...
    it('should record cost in UPDATE path when providerName and tokens > 0', async () => {
      ...
      await logRequest(
        createBaseParams({ logId: 'existing-log-1', providerName: 'openai', inputTokens: 10, outputTokens: 5 })
      );
    ...
    it('should skip cost recording in UPDATE path when providerName is missing', async () => {
      ...
      await logRequest(
        createBaseParams({ logId: 'existing-log-1', providerName: undefined, inputTokens: 10, outputTokens: 5 })
      );
  ...
  describe('requestBody with stream content', () => {
    it('should handle streaming response with streamContent/streamProgress', async () => {
      await logRequest(
        createBaseParams({
          streaming: true,
          responseBody: { streamContent: { chunks: ['abc'] }, streamProgress: { chunksProcessed: 1 } },
        })
      );

...
const mockExecutorProviderErrorPassthrough = mock(async () => new Response('error', { status: 500 }));
...
describe('handleAnthropicMessages', () => {
  beforeEach(() => {
    ...
    mockHandleNonStreamingResponse.mockImplementation(async () => new Response('{}', { status: 200 }));
    mockHandleStreamingResponse.mockImplementation(async () => new Response('stream', { status: 200 }));
    mockHandleGatewayError.mockImplementation(async () => new Response('gateway error', { status: 500 }));
  });

...
function getAbortControllers() {
  return (logEventBus as unknown as { abortControllers: Map<string, AbortController> }).abortControllers;
}
...
describe('logEventBus', () => {
  ...
  it('emitLog waiting → adds to activeStreams', () => {
    const event: LiveStreamEvent = { event: 'waiting', logId: 'log-1', modelName: 'gpt-4', providerName: 'openai', startTime: Date.now(), incomingProtocol: 'openai' };
  ...
  it('emitLog started → adds to activeStreams and overwrites waiting', () => {
    const waitingEvent: LiveStreamEvent = { event: 'waiting', logId: 'log-1', modelName: 'gpt-4', providerName: 'openai', startTime: Date.now(), incomingProtocol: 'openai' };
    const startedEvent: LiveStreamEvent = { event: 'started', logId: 'log-1', modelName: 'gpt-4', providerName: 'openai', startTime: Date.now(), incomingProtocol: 'openai' };
  ...
  it('emitLog chunk → adds to activeStreams', () => {
    const event: LiveStreamEvent = { event: 'chunk', logId: 'log-1', outputTokens: 10, totalChunks: 5, hasThinking: false, elapsedMs: 1000 };
    logEventBus.emitLog(event);
  ...
  it('emitLog completed → removes from activeStreams', () => {
    ...
    const completedEvent: LiveStreamEvent = { event: 'completed', logId: 'log-1', status: 'success', inputTokens: 100, outputTokens: 50, responseTimeMs: 2000 };
  ...
  it('emitLog aborted → removes from activeStreams', () => {
    ...
    const abortedEvent: LiveStreamEvent = { event: 'aborted', logId: 'log-1', reason: 'client_disconnect' };
  ...
  it('activeStreams tracks lifecycle through waiting → started → chunk → completed', () => {
    const waiting: LiveStreamEvent = { event: 'waiting', logId: 'log-1', modelName: 'gpt-4', providerName: 'openai', startTime: Date.now(), incomingProtocol: 'openai' };
    const started: LiveStreamEvent = { event: 'started', logId: 'log-1', modelName: 'gpt-4', providerName: 'openai', startTime: Date.now(), incomingProtocol: 'openai' };
    const chunk: LiveStreamEvent = { event: 'chunk', logId: 'log-1', outputTokens: 5, totalChunks: 1, hasThinking: false, elapsedMs: 500 };
    const completed: LiveStreamEvent = { event: 'completed', logId: 'log-1', status: 'success', inputTokens: 10, outputTokens: 5, responseTimeMs: 1500 };
  ...
  it('multiple streams tracked independently', () => {
    const stream1: LiveStreamEvent = { event: 'waiting', logId: 'log-1', modelName: 'gpt-4', providerName: 'openai', startTime: Date.now(), incomingProtocol: 'openai' };
    const stream2: LiveStreamEvent = { event: 'started', logId: 'log-2', modelName: 'claude-3', providerName: 'anthropic', startTime: Date.now(), incomingProtocol: 'anthropic' };
    ...
    const completed: LiveStreamEvent = { event: 'completed', logId: 'log-1', status: 'success', inputTokens: 10, outputTokens: 5, responseTimeMs: 1000 };
    logEventBus.emitLog(completed);

  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <KeyDisplay
          keyValue={key.key}
          showKey={display.showKeyValue[key.id]}
          copied={display.copiedKey === key.id}
          onToggleShow={() => display.onToggleShow(key.id)}
          onCopy={() => display.onCopy(key.key, key.id)}
        />
      </TableCell>
      <TableCell>
      ...
      <TableCell>
        <span
          className={`text-sm ${key.expiresAt && new Date(key.expiresAt) < new Date() ? 'text-destructive' : 'text-muted-foreground'}`}
        >
      ...
      <TableCell className="text-right">
        <div className="flex justify-end gap-2">
          <Button
            ...
            size="sm"
            title="查看用量统计"
            onClick={() => actions.onShowStats(key.id)}
          >
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => actions.onReset(key.id)}
            title="重置密钥"
          >
            <RefreshCw className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => actions.onEdit(key.id)}>
            <Pencil className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => actions.onDelete(key.id, key.name)}>

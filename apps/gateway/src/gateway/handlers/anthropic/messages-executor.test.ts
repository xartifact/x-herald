...
function createExecutorConfig(overrides: Partial<import('./messages-executor').AnthropicExecutorConfig> & { isStreaming?: boolean; isPassthroughEnabled?: boolean } = {}): import('./messages-executor').AnthropicExecutorConfig {
  ...
  return {
    ...
    retryCount: 0,
    ...restOverrides,
    ...(isStreaming !== undefined ? { req: { rawBody: { model: 'claude-3' }, standardReq: { model: 'claude-3', messages: [] }, standardRequestBody: { model: 'claude-3', messages: [] }, virtualKey: createTestVirtualKey(), clientRequestHeaders: {}, clientIp: '127.0.0.1', userAgent: 'test-agent', clientType: 'unknown', requestPath: '/v1/messages', requestMethod: 'POST', conversationId: 'conv-1', isStreaming, incomingProtocol: 'anthropic' as const, startTime: Date.now(), requestId: 'test-req-1' } } : {}),
  };
}
...
describe('AnthropicMessagesExecutor', () => {
  beforeEach(() => {
    ...
    mockHandleProviderErrorPassthrough.mockImplementation(async () => new Response('error', { status: 500 }));

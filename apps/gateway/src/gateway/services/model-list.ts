    ...
    const capRows =
      amIds.length > 0
        ? await db
            ...
            .where(
              and(
                sql`${modelRoutes.accessModelIds} && ARRAY[${sql.join(
                  amIds.map((id) => sql`${id}`),
                  sql`, `,
                )}]::text[]`,
    ...
    for (const row of capRows) {
      ...
      if (!existing) {
        capMap.set(row.amId, {
          ...
          maxOutputTokens: Number(
            cap.maxOutputTokens ?? cap.max_output_tokens ?? cap.maxTokens ?? 0,
          ),
        })
      } else {
        ...
        existing.contextWindow = Math.max(
          existing.contextWindow,
          Number(cap.contextWindow ?? cap.context_window ?? 0),
        )
        existing.maxOutputTokens = Math.max(
          existing.maxOutputTokens,
          Number(cap.maxOutputTokens ?? cap.max_output_tokens ?? cap.maxTokens ?? 0),
        )
      }
    }

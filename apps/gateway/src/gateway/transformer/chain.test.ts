...
function createMockTransformer(name: string, overrides: Partial<Transformer> = {}): Transformer {
  return {
    name,
    ...
    normalizeResponse: mock(
      async (res: Response, ctx: TransformerContext) => res as unknown as StandardResponse,
    ),
...
describe('TransformerChain', () => {
  ...
  describe('normalize', () => {
    ...
    it('passes output of one transformer as input to next', async () => {
      const t1 = createMockTransformer('t1', {
        normalizeRequest: mock(
          async (req: unknown, _ctx: TransformerContext) =>
            ({ ...(req as object), step: 1 }) as unknown as StandardRequest,
        ),
      })
      const t2 = createMockTransformer('t2', {
        normalizeRequest: mock(
          async (req: unknown, _ctx: TransformerContext) =>
            ({ ...(req as object), step: 2 }) as unknown as StandardRequest,
        ),
      })
      getTransformerMock.mockImplementation((name: string) => {
        if (name === 't1') return t1
  ...
  describe('adapt', () => {
    ...
    it('passes normalized request to adaptRequest', async () => {
      const t1 = createMockTransformer('t1', {
        normalizeRequest: mock(
          async (req: StandardRequest) => ({ ...req, normalized: true }) as StandardRequest,
        ),
      })
      getTransformerMock.mockReturnValue(t1)
      ...
      const adaptCall = (t1.adaptRequest as unknown as { mock: { calls: unknown[][] } }).mock
        .calls[0]
    ...
    it('throws "No adapter found in chain" when no adaptRequest exists', async () => {
      ...
      const chain = new TransformerChain(['t1'], 'request')
      const ctx = createContext()
      await expect(chain.adapt({ model: 'test' } as StandardRequest, ctx)).rejects.toThrow(
        'No adapter found in chain',
      )
    })
  ...
  describe('normalizeResponse', () => {
    ...
    it('throws when no transformer has normalizeResponse', async () => {
      ...
      const ctx = createContext()
      await expect(chain.normalizeResponse(new Response('body'), ctx)).rejects.toThrow(
        'No response normalizer found in chain',
      )
    })
  ...
  describe('adaptResponse', () => {
    ...
    it('throws when no transformer has adaptResponse', async () => {
      ...
      const ctx = createContext()
      await expect(
        chain.adaptResponse({ status: 200 } as unknown as StandardResponse, ctx),
      ).rejects.toThrow('No response adapter found in chain')
    })
    ...
    it('re-throws transformer error', async () => {
      ...
      await expect(
        chain.adaptResponse({ status: 200 } as unknown as StandardResponse, ctx),
      ).rejects.toThrow('adaptResponse error')
    })
  })

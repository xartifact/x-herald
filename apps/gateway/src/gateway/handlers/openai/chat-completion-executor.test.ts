...
const mockHandleProviderErrorPassthrough = mock(async () => new Response('passthrough error', { status: 500 }));
...
import { createTestProvider, createTestModelGroup, createTestModelInstance, createTestVirtualKey } from '../../../test/factories';
...
function createMockContext() {
  return {
    ...
    get: mock(() => undefined),
    json: mock((body: unknown, status?: number) => new Response(JSON.stringify(body), { status: status ?? 200 })),
...
function createBaseExecutorConfig(overrides: Record<string, unknown> = {}): ExecutorConfig {
  ...
  return {
    ...
    candidate: {
      ...
      provider,
      group,
      mapping: (overrides.mapping ?? { modelName: 'gpt-4', isMapped: true, originalModel: 'gpt-4', mappingType: 'virtual' }),
      decision: (overrides.decision ?? { strategy: 'priority' }),
      matchedRule: (overrides.matchedRule ?? { id: 'rule-1', name: 'Test Rule', priority: 1 }),
    },
    req: {
      rawBody: (overrides.rawBody ?? { model: 'gpt-4', messages: [{ role: 'user', content: 'hello' }] }),
      standardReq: (overrides.standardReq ?? { model: 'gpt-4', messages: [{ role: 'user', content: 'hello' }] }),
      standardRequestBody: (overrides.standardRequestBody ?? { model: 'gpt-4', messages: [{ role: 'user', content: 'hello' }] }),
      virtualKey,
      clientRequestHeaders: (overrides.clientRequestHeaders ?? { 'content-type': 'application/json' }),
...
describe('ChatCompletionCandidateExecutor', () => {
  beforeEach(() => {
    ...
    const allMocks = [
      ...
      mockCircuitBreakerRegistry.recordFailure, mockCircuitBreakerRegistry.recordSuccess,
      mockHandleGatewayError, mockHandleProviderError, mockHandleProviderErrorPassthrough,
      mockLogStartAsync, mockMarkLogAsFailed, mockLogEventBus.emitLog,
      mockEgressTransformer.adaptRequest, mockBuildHeaders,
    ];
  ...
  it('beforeFetch() emits waiting event for streaming requests', async () => {
    ...
    expect(mockLogEventBus.emitLog).toHaveBeenCalledWith(expect.objectContaining({ event: 'waiting' }));
  });

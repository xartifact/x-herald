...
function withDbTimeout<T>(p: Promise<T>): Promise<T> {
  return Promise.race([
    ...
    new Promise<never>((_, rej) =>
      setTimeout(
        () => rej(new Error(`DB write timed out after ${DB_WRITE_TIMEOUT_MS}ms`)),
        DB_WRITE_TIMEOUT_MS,
      ),
    ),
  ])
...
async function createStreamLog(
  ...
  try {
    ...
    const result = await db.transaction(async (trx) => {
      const logResult = await withDbTimeout(
        trx
          .insert(requestLogs)
          .values(buildLogInsertValues(params))
          .returning({ id: requestLogs.id }),
      )
      const logId = logResult[0].id

      const attemptResult = await withDbTimeout(
        trx
    ...
    logger.debug(
      { logId: result.logId, attemptId: result.attemptId },
      'Stream log and attempt created',
    )
    return result
  } catch (error) {
...
async function createStreamLogById(
  ...
  await db.transaction(async (trx) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await withDbTimeout(
      trx.insert(requestLogs).values({ id: logId, ...buildLogInsertValues(params) } as any),
    )
    await withDbTimeout(
...
export function logStartAsync(params: StreamLogParams): LogStartResult {
  ...
  createStreamLogById({ ...params, isStream: false }, logId, attemptId).catch((err) =>
    logger.error({ err, logId }, '[Log] 异步初始日志写入失败'),
  )
  ...
  return {
    ...
    metadata: {
      ...
      performance: {
        ...
        ...(params.ttfbToFirstThinkingMs != null && {
          ttfbToFirstThinkingMs: params.ttfbToFirstThinkingMs,
        }),
...
export async function finalizeStreamLog(
  ...
  const doFinalize = async (trx: DbClient) => {
    ...
    await withDbTimeout(
      trx
        .update(requestLogs)
        .set(buildFinalizeValues(params, responseTimeMs))
        .where(eq(requestLogs.id, logId)),
    )
    ...
    await db.transaction(async (trx) => {
      ...
      if (attemptId && !attemptId.startsWith('temp-')) {
        await withDbTimeout(
          trx
            .update(requestAttempts)
            .set({ status: 'failure', statusCode: error.statusCode })
...
export async function markAttemptFailed(params: {
  ...
  try {
    ...
    logger.debug(
      { logId, attemptId, statusCode: params.statusCode, failoverReason: params.failoverReason },

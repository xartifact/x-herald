...
async function runPgliteMigrations(
  pgliteClient: { exec: (sql: string) => Promise<unknown>; query: (sql: string, params?: unknown[]) => Promise<{ rows: Array<{ hash: string }> }> },
  ...
  const migrationFiles = fs.readdirSync(migrationsFolder).filter((f) => extname(f) === ".sql").sort();
  ...
  for (const file of migrationFiles) {
    ...
    try {
      ...
      await pgliteClient.query('INSERT INTO "__drizzle_migrations" (hash, created_at) VALUES ($1, $2)', [hash, Date.now()]);
      ...
      if (msg.includes("already exists") || msg.includes("duplicate") || msg.includes("does not exist")) {
        await pgliteClient.query('INSERT INTO "__drizzle_migrations" (hash, created_at) VALUES ($1, $2)', [hash, Date.now()]);
        logger.trace({ file, error: msg.split("\n")[0] }, "[DB] Migration already applied or irrelevant");
        skipped++;
        appliedHashes.add(hash);
      } else {

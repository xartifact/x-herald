...
export function extractMessageSequence(
  ...
  messages.forEach((msg: any, index: number) => {
    ...
    if (typeof msg.content === 'string') {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      length = msg.content.reduce(
        (sum: number, part: any) => sum + (part.text ? part.text.length : 0),
        0,
      )
    }
...
function extractToolResultsFromRequest(
  standardRequestBody?: unknown,
): Array<{ toolName?: string; callId?: string; result: unknown }> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return body.messages
    ...
    .map((msg: any) => ({ toolName: msg.name, callId: msg.tool_call_id, result: msg.content }))
}

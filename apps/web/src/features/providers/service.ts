...
const ProtocolsSchema = z.record(z.string(), ProtocolConfigSchema).refine(
...
export async function listProviders() {
  return getDatabase().select().from(providers).orderBy(desc(providers.createdAt))
}
...
export async function createProvider(data: CreateProviderCommand) {
  const db = getDatabase()
  const rows = await db.insert(providers).values({
    name: data.name,
...
export async function updateProvider(id: string, data: UpdateProviderCommand) {
  const db = getDatabase()
  const existing = await getProvider(id)
  if (!existing) return null
  const rows = await db.update(providers).set({
...
export async function toggleProvider(id: string) {
  ...
  const existing = await getProvider(id)
  if (!existing) return null
  const rows = await db.update(providers).set({ enabled: !existing.enabled, updatedAt: new Date() }).where(eq(providers.id, id)).returning()
  logger.info({ providerId: id, enabled: rows[0].enabled }, 'Provider toggled')
  return rows[0] ?? null
}

export async function getThinkingMappings(id: string) {
...
export async function updateThinkingMappings(id: string, data: ThinkingMappingData) {
  const db = getDatabase()
  const provider = await getProvider(id)
  if (!provider) return null
  ...
  await db.update(providers).set({ protocols: updatedProtocols, updatedAt: new Date() }).where(eq(providers.id, id))
...
export async function fetchRemoteModels(id: string): Promise<FetchModelsResult> {
  const provider = await getProvider(id)
  ...
  const db = getDatabase()
  const existing = await db.select({ actualModelName: modelInstances.actualModelName }).from(modelInstances).where(eq(modelInstances.providerId, id))
...
export async function syncModels(id: string, data: SyncModelsCommand): Promise<SyncModelsResult> {
  const provider = await getProvider(id)
  if (!provider) return { ok: false, code: 'NOT_FOUND' }
  if (!provider.enabled) return { ok: false, code: 'DISABLED' }

 ...
 * 获取配置值（带缓存）
 */
export async function getConfig<T>(key: string, defaultValue: T, db?: Database): Promise<T> {
  // 如果缓存已初始化且存在该配置，直接返回
 ...
 * 设置配置值（更新缓存和数据库）
 */
export async function setConfig<T>(key: string, value: T, description?: string, db?: Database): Promise<void> {
  const database = db ?? getDatabase();
  ...
  if (existing.length > 0) {
    ...
    await database.insert(gatewayConfigs).values({
      key,
      value: value as any,
      description,
    });
 ...
 * 初始化配置缓存
 */
export async function initConfigCache(db?: Database): Promise<void> {
  const database = db ?? getDatabase();
  const configs = await database.select().from(gatewayConfigs);

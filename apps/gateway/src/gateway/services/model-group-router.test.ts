...
describe('ModelGroupRouter integration', () => {
  beforeAll(async () => {
    ...
    const disabledInstRes = await authenticatedRequest(
      app,
      'POST',
      '/api/model-groups/instances',
      token,
      {
        body: {
          providerId,
    ...
    await authenticatedRequest(
      app,
      'PATCH',
      `/api/model-groups/instances/${disabledInstanceId}/toggle`,
      token,
    )

    // Instance 4: attached to disabled provider
    const disabledProviderInstRes = await authenticatedRequest(
      app,
      'POST',
      '/api/model-groups/instances',
      token,
    ...
    const disabledProviderInstData = (await disabledProviderInstRes.json()) as {
      data: { id: string }
    }
  ...
  it('throws ModelNotFoundError for non-existent groupId', async () => {
    await expect(
      modelGroupRouter.routeCandidatesByGroupId(
  ...
  it('routeByGroupId throws for group with no instances', async () => {
    await expect(modelGroupRouter.routeByGroupId(emptyGroupId, defaultContext)).rejects.toThrow(
  ...
  it('getModelGroupDetail returns null for non-existent groupId', async () => {
    const detail = await modelGroupRouter.getModelGroupDetail(
      '00000000-0000-0000-0000-000000000000',
    )

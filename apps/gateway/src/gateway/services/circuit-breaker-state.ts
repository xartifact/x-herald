  ...
  async recordFailure(instanceId: string, meta?: CircuitBreakerMeta): Promise<void> {
    ...
    if (s.state === 'half_open') {
      ...
      if (decision.nextState === 'open') {
        ...
        s.openUntil = 0
        logger.warn(
          { instanceId, failures: s.failures, tripCount: s.tripCount },
  ...
  > {
    ...
    for (const [instanceId, s] of this.states.entries()) {
      ...
      if (s.state === 'cooldown' && s.cooldownUntil > 0)
        remainingMs = Math.max(0, s.cooldownUntil - this.now())
      result.push({
        instanceId,
        state: s.state,
...
export async function recoverCircuitBreakerState(
  now: () => number = () => Date.now(),
): Promise<void> {
  ...
  try {
    ...
    const openedEvents = await db
      ...
      .where(
        and(
          ...
          gt(circuitBreakerEvents.openUntil, new Date(now())),
        ),
      )
    ...
    for (const [instanceId, event] of latestOpened) {
      if (event.openUntil) {
        ...
        circuitBreakerRegistry.restoreOpenState(
          instanceId,
          event.openUntil,
          event.failureCount,
          event.tripCount ?? 0,
    ...
    const cooldownEvents = await db
      ...
      .where(
        and(
          ...
          gt(circuitBreakerEvents.cooldownUntil, new Date(now())),
        ),
      )
      .orderBy(desc(circuitBreakerEvents.createdAt))
    ...
    for (const [instanceId, event] of latestCooldown) {
      if (!latestOpened.has(instanceId) && event.cooldownUntil) {
        ...
        circuitBreakerRegistry.restoreCooldownState(
          instanceId,
          event.cooldownUntil,
          event.failureCount,
          event.tripCount ?? 0,
    ...
    logger.warn(
      { err },
      '[CircuitBreaker] Failed to recover circuit breaker state from DB, skipping',
    )
  }
}

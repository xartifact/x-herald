...
export async function listModelRoutes(accessModelId?: string, db?: Database) {
  ...
  let query = database.select(routeSelect).from(modelRoutes).$dynamic()
...
export async function deleteModelRoute(id: string, db?: Database) {
  const database = db ?? getDatabase()
  const [deleted] = await database.delete(modelRoutes).where(eq(modelRoutes.id, id)).returning()
  return deleted ?? null
}

export async function toggleModelRoute(id: string, db?: Database) {
  const database = db ?? getDatabase()
  const existing = await database.select().from(modelRoutes).where(eq(modelRoutes.id, id)).limit(1)

...
function getCache(): Map<string, PatternEntry> {
  const globalCache = (globalThis as Record<string, unknown>)[ERROR_PATTERNS_CACHE_KEY] as
    | Map<string, PatternEntry>
    | undefined;
  if (globalCache) return globalCache;
  const created = new Map<string, PatternEntry>();
  (globalThis as Record<string, unknown>)[ERROR_PATTERNS_CACHE_KEY] = created;
  return created;
}

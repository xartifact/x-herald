 ...
 */
export function createModelRemapStream(originalModelName: string): TransformStream<Uint8Array, Uint8Array> {
 ...
 */
export function extractUsageFromChunk(data: string): { prompt_tokens?: number; completion_tokens?: number } | null {
  try {
    const json = JSON.parse(data);

...
export async function fetchGroupIdsByInstanceIds(instanceIds: string[]): Promise<Map<string, string[]>> {
...
export async function setInstanceGroups(instanceId: string, groupIds: string[]): Promise<void> {
  const db = getDatabase()
  await db.delete(modelGroupMemberships).where(eq(modelGroupMemberships.instanceId, instanceId))
  if (groupIds.length > 0) {
    await db.insert(modelGroupMemberships).values(groupIds.map((gid) => ({ groupId: gid, instanceId })))
  }
}
...
export async function listInstances() {
  ...
  const groupIdsMap = await fetchGroupIdsByInstanceIds(instances.map((i) => i.id))
  return attachGroupIds(instances, groupIdsMap)
}

export async function listGroups() {
  return getDatabase().select().from(modelGroups).orderBy(desc(modelGroups.createdAt))
}
...
export async function createInstance(data: CreateInstanceData) {
  const db = getDatabase()
  ...
  if (groupIds.length > 0) {
    const groups = await db.select({ id: modelGroups.id }).from(modelGroups).where(inArray(modelGroups.id, groupIds))
    if (groups.length !== groupIds.length) return { error: 'One or more model groups not found' as const }
  }

  const provider = await db.select().from(providers).where(eq(providers.id, data.providerId)).limit(1)
  ...
  const [instance] = await db.insert(modelInstances).values(insertValues).returning()

  if (groupIds.length > 0) {
    await db.insert(modelGroupMemberships).values(groupIds.map((gid) => ({ groupId: gid, instanceId: instance.id })))
  }
...
export async function updateInstance(id: string, data: UpdateInstanceData) {
  const db = getDatabase()
  const [updated] = await db.update(modelInstances).set({
  ...
  if (data.groupIds !== undefined || data.groupId !== undefined) {
    ...
    await setInstanceGroups(id, groupIds)
  }

  const groupIdsMap = await fetchGroupIdsByInstanceIds([id])
  return attachGroupIds([updated], groupIdsMap)[0]
}

export async function deleteInstance(id: string) {
  const db = getDatabase()
  const [deleted] = await db.delete(modelInstances).where(eq(modelInstances.id, id)).returning()
  return deleted ?? null
}

export async function setInstanceGroupsById(id: string, groupIds: string[]) {
  const db = getDatabase()
  const instance = await db.select().from(modelInstances).where(eq(modelInstances.id, id)).limit(1)
  if (instance.length === 0) return null

  const resolvedGroupIds = groupIds ?? []
  if (resolvedGroupIds.length > 0) {
    const groups = await db.select({ id: modelGroups.id }).from(modelGroups).where(inArray(modelGroups.id, resolvedGroupIds))
    if (groups.length !== resolvedGroupIds.length) return { error: 'One or more model groups not found' as const }
  }

  await setInstanceGroups(id, resolvedGroupIds)
...
export async function assignInstance(id: string, groupId: string | null) {
  const db = getDatabase()
  const instance = await db.select().from(modelInstances).where(eq(modelInstances.id, id)).limit(1)
  if (instance.length === 0) return { error: 'Model instance not found' as const }

  if (groupId) {
    const group = await db.select({ id: modelGroups.id }).from(modelGroups).where(eq(modelGroups.id, groupId)).limit(1)
...
export async function toggleInstance(id: string) {
  const db = getDatabase()
  const instance = await db.select().from(modelInstances).where(eq(modelInstances.id, id)).limit(1)
  if (instance.length === 0) return null
  const [updated] = await db.update(modelInstances).set({ enabled: !instance[0].enabled, updatedAt: new Date() }).where(eq(modelInstances.id, id)).returning()
  const groupIdsMap = await fetchGroupIdsByInstanceIds([id])
  return attachGroupIds([updated], groupIdsMap)[0]
}

export async function reorderInstances(instanceIds: string[]): Promise<void> {
  const db = getDatabase()
  for (let i = 0; i < instanceIds.length; i++) {
    await db.update(modelInstances).set({ priority: i, updatedAt: new Date() }).where(eq(modelInstances.id, instanceIds[i]))
  }
}
...
export async function createGroup(data: GroupData) {
  ...
  const [group] = await db.insert(modelGroups).values(insertValues).returning()
  logger.info({ groupId: group.id, name: group.name }, 'Model group created')
  return group
}

export async function updateGroup(id: string, data: Partial<GroupData>) {
  const db = getDatabase()
  const [updated] = await db.update(modelGroups).set({
    ...(data.name !== undefined && { name: data.name }),
...
export async function deleteGroup(id: string) {
  const db = getDatabase()
  const [deleted] = await db.delete(modelGroups).where(eq(modelGroups.id, id)).returning()
  return deleted ?? null
}
...
export async function toggleGroup(id: string) {
  ...
  const [updated] = await db.update(modelGroups).set({ enabled: !group[0].enabled, updatedAt: new Date() }).where(eq(modelGroups.id, id)).returning()
  return updated ?? null
}

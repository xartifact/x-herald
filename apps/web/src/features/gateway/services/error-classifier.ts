...
export function normalizeProviderErrorMessage(rawMessage: string): { message: string; code: string } {
...
export async function parseProviderError(response: Response): Promise<{ error?: { message?: string }; [key: string]: unknown }> {
  ...
  if (contentType.includes('text/event-stream') || text.startsWith('data:')) {
    ...
    for (const line of lines) {
      if (line.startsWith('data:')) {
        ...
        try { return JSON.parse(data); } catch { /* next line */ }
      }
    }
